...
CodeWarning::CodeWarning(bool fix, uint8_t order,
...
void CodeWarning::Initialize()
{
   ...
   //
   const int U = 0;  // fix does not change code
   const int D = 2;  // fix deletes a line of code
   const int C = 2;  // fix creates a line of code
   const int E = 4;  // fix erases within a line
   const int R = 4;  // fix replaces within a line
   const int I = 4;  // fix inserts within a line
   const int J = 6;  // fix removes a line feed
   const int S = 6;  // fix inserts a line feed
   const int X = 8;  // fix not implemented
   const auto F = false;
   const auto T = true;

   //  With regard to sort order
   ...
   Attrs.insert(WarningPair(DefaultAssignment,
      CodeWarning(F, X, F, F,
   ...
   Attrs.insert(WarningPair(RemoveLineBreak,
      CodeWarning(T, J, F, F,
      "Line can be merged with the next one and be under the length limit")));
