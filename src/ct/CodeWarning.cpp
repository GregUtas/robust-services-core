...
WarningAttrs::WarningAttrs(bool fixable, bool disabled, c_string expl) :
   fixable_(fixable),
   disabled_(disabled),
   expl_(expl)
{
   ...
   status_(NotSupported)
{
   ...
   const auto& attrs = Attrs_.at(warning);
   if(attrs.disabled_)
      status_ = Disabled;
   else if(attrs.fixable_)
      status_ = NotFixed;
}
